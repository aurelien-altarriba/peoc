...
$(document).ready(function() {
  ...
  // POINTS DE VIGILANCE
	...
	var btPointInteret = L.control({position: 'topleft'});
	btPointInteret.onAdd = function (map) {
	  var container = L.DomUtil.create('div', 'leaflet-bar leaflet-control leaflet-control-custom bt_custom_map');

    container.style.backgroundColor = 'white';
    container.style.width = '30px';
    container.style.height = '30px';
		container.style.cursor = 'pointer';
		container.innerHTML = '<img src="../image/pi.png" style="width: 80%;"/>';

    container.onclick = function() {
      $('#map').css('cursor', 'pointer !important');
    }

		return container;
	};
	btPointInteret.addTo(map);

	var btPointVigilance = L.control({position: 'topleft'});
	btPointVigilance.onAdd = function (map) {
	  var container = L.DomUtil.create('div', 'leaflet-bar leaflet-control leaflet-control-custom bt_custom_map');
    ...
    container.style.height = '30px';
		container.style.cursor = 'pointer';
		container.innerHTML = '<img src="../image/pv.png" style="width: 80%;"/>';
  ...
  L.Control.Watermark = L.Control.extend({
  	...
  	mapClicAjouter: function (e) {
  		...
  		$('#titre_form_marqueur').text('Ajouter un marqueur');
